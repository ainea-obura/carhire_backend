        ...
        <div class="card-body">
            <div class="table-responsive">
                @if (count($hire) > 0)
                        ...
                        <tbody>
                            @foreach ($hire as $hire)
                                <tr>
                                    ...
                                    <td>{{ $hire->amount }}</td>
                                    <td>
                                        <a href="{{ route('hire.show', $hire->id) }}"
                                            class="btn btn-warning btn-sm float-left mr-1"
                                            style="height:30px; width:30px;bhire-radius:50%" data-toggle="tooltip"
                                        ...
                                        <form method="POST" action="{{ route('hire.destroy', [$hire->id]) }}">
                                            @csrf
                                            @method('delete')
                                            <button class="btn btn-danger btn-sm dltBtn" data-id={{ $hire->id }}
                                                style="height:30px; width:30px;bhire-radius:50%" data-toggle="tooltip"
                    ...
                    {{-- <span style="float:right">{{$hire->links()}}</span> --}}
                @else
